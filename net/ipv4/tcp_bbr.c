...
static int bbr_update_ecn_alpha(struct sock *sk)
{
	...
	if (!bbr->ecn_eligible && (bool)bbr_can_use_ecn(sk) &&
	    (bool)bbr_param(sk, ecn_factor) &&
				      ...
				      const struct rate_sample *rs)
{
	...
	if (rs->delivered_ce > 0 && rs->delivered > 0 &&
	    bbr->ecn_eligible && (bool)bbr_param(sk, ecn_thresh)) {
				    ...
				    const struct rate_sample *rs)
{
	...
	/* ECN response. */
	if (bbr->ecn_in_round && (bool)bbr_param(sk, ecn_factor)) {
